%perform the image processing for the nissl sagittal slices
function annotate_translate_sagittal_nissl61(startdiroffset)
dirlist=dir ('./annotate_sagittal_nissl/a*.dat');

%check if directory exists, otherwise create
mkdir('result_vectors_sagittal_nissl61')
mkdir('baselines61')
mkdir('cumulative61')
mkdir('alldots61')
mkdir('heightdots61')
...
for count00=startdiroffset:min(startdiroffset+20000, length(dirlist))
    ...
    filestr1=strcat(strcat('images_sagittal_nissl/',str1),'.jpg');
    ...
    mapname=strcat(strcat('result_vectors_sagittal_nissl61/',str1),'.dat');
    ...
    %now read in the marked points
    filestr2=strcat('annotate_sagittal_nissl/',name1);
