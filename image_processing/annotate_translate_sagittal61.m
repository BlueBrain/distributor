%perform the image processing for the sagittal slices
function annotate_translate_sagittal61(startdiroffset)
dirlist=dir ('./annotate_sagittal/a*.dat'); %all neuron marker


%check if directory exists, otherwise create
mkdir('result_vectors_sagittal61')
...
for count00=startdiroffset:min(startdiroffset+20000, length(dirlist))
    ...
    filestr1=strcat(strcat('images_sagittal/',str1),'.jpg');
    ...
    mapname=strcat(strcat('result_vectors_sagittal61/',str1),'.dat');
    ...
    pointsL4=[];
    filestr2=strcat('annotate_sagittal_L4/',strcat('L4_',name1));
    if (exist(filestr2))
      disp 'reading L4'
    ...
    %now read in the marked points
    filestr2=strcat('annotate_sagittal/',name1);
